...
enum : uint32_t {
    ...
    SCRIPT_VERIFY_INTERNALKEY = (1U << 22),

    // OP_PAIRCOMMIT, validation (BIP-442)
    SCRIPT_VERIFY_PAIRCOMMIT = (1U << 23),

    // reserved for OP_CAT, validation (BIP-347)
    SCRIPT_VERIFY_CAT = (1U << 24),

    // reserved for OP_CHECKCONTRACTVERIFY, validation (BIP-???)
    SCRIPT_VERIFY_CHECKCONTRACTVERIFY = (1U << 25),

    // discourage upgradable OP_CHECKTEMPLATEVERIFY (BIP-119) hashes
    SCRIPT_VERIFY_DISCOURAGE_UPGRADABLE_CHECKTEMPLATEVERIFY = (1U << 26),
    ...
    SCRIPT_VERIFY_DISCOURAGE_CAT = (1U << 30),

    // reserved for discourage OP_CHECKCONTRACTVERIFY (BIP-???)
    SCRIPT_VERIFY_DISCOURAGE_CHECKCONTRACTVERIFY = (1U << 31),
